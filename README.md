...
- **Frontend (`public/`):** Interfaz de usuario simple para la subida de archivos (Drag & Drop y botón).
- **Backend (`server.js`):** El servidor Express que maneja las rutas y actúa como el **Motor de Migración**.
- **Capa de Persistencia:** Base de datos MySQL.

---

## ✨ Características y Funcionalidades del Motor

El motor de migración encapsulado en `server.js` incluye los siguientes módulos avanzados para garantizar una migración de alta calidad:

| Módulo                      | Descripción                                                                                                                                                                                            | Cumplimiento |
| :-------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------: |
| **Parsing Robusto**         | Lee archivos CSV, JSON y XML. Implementa heurísticas para detectar delimitadores CSV comunes (`,`, `;`, `\t`, etc.) y maneja archivos con múltiples espacios.                                          |    **✅**    |
...
| **Promoción de Tipo**       | Rastrea la longitud máxima de las cadenas y **promueve automáticamente** `VARCHAR(255)` a `TEXT` si es necesario, evitando el error _"Data too long"_.                                                 |    **✅**    |
...
- **Importante:** La función `initializeDb()` en `server.js` creará automáticamente la base de datos `migration_db` si no existe al iniciar el servidor.
...
const DB_CONFIG =  {
    host: "localhost",
    user: "root",
    password: "TU_CONTRASEÑA_AQUÍ", // <--- ¡Asegúrate de cambiar esto!
...
Al finalizar la migración, la interfaz mostrará un resumen. Los archivos se guardarán localmente en la carpeta donde ejecutaste node `server.js`:

- DDL (`[tabla]_[timestamp]_schema.sql`): El script SQL generado para crear la tabla.
